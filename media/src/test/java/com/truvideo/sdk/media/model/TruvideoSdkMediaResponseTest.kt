...
class TruvideoSdkMediaResponseTest {
    ...
    fun `fromJson parses valid JSON correctly`() {
        ...
        assertEquals(60.0F, result.transcriptionLength)
        assertEquals(2, result.tags.map.size)
        assertEquals("value1", result.tags.map["tag1"])
        assertEquals("value2", result.tags.map["tag2"])
    }
    ...
    fun `fromJson handles empty tags correctly`() {
        ...
        assertEquals("http://example.com/transcription", result.transcriptionUrl)
        assertEquals(60.0F, result.transcriptionLength)
        assertEquals(0, result.tags.map.size)
    }
}
