...
internal class TruvideoSdkMediaServiceImpl(
    ...
    override suspend fun createMedia(
        ...
        val body = JSONObject().apply {
            ...
            put("resolution", "LOW")
            put("size", size)
            put("tags", JSONObject().apply { for (tag in tags.map.entries) put(tag.key, tag.value) })
        ...
        if (response == null || !response.isSuccess) {
            ...
            throw TruvideoSdkMediaException("Error creating media")
        }
        ...
        if (media == null) {
            throw TruvideoSdkMediaException("Error fetching media")
        }
    ...
    override suspend fun fetchAll(
        ...
        val baseUrl = sdk_common.configuration.environment.baseUrl
        val url = buildSearchUrl(baseUrl, pageNumber, pageSize)
        val response = sdk_common.http.post(
            url = url,
            headers = headers,
            retry = true,
        ...
        for (i in 0 until contentArray.length()) {
            ...
            try {
                ...
                Log.d("TruvideoSdkMedia", "Error parsing media response ${exception.localizedMessage}")
            }
        }
