...
internal class TruvideoSdkMediaFileUploadEngine(
    ...
    suspend fun upload(id: String, callback: TruvideoSdkMediaFileUploadCallback) {
        ...
        // Get credentials
        val credentials = sdk_common.auth.getSettings()?.credentials
        if (credentials == null) {
            logAdapter.addLog(
                eventName = "event_media_file_upload_request_start",
        ...
        // validate file type
        val uri = Uri.fromFile(File(entity.filePath))
        val type = entity.type
        if (type == TruvideoSdkMediaFileUploadRequestMediaType.UNKNOWN) {
            val mimeType = FileUriUtil.getMimeType(context, uri)
            logAdapter.addLog(
                eventName = "event_media_file_upload_request_start",
                message = "Invalid file type. Path: ${entity.filePath}. MimeType: $mimeType",
                severity = TruvideoSdkLogSeverity.ERROR
            )
        ...
        val durationInSeconds = if (entity.type.withDuration) {
            (entity.durationMilliseconds ?: 0L) / 1000L
        } else {
            null
        }
                            ...
                            mutex.withLock {
                                try {
                                    ...
                                    val media = mediaService.createMedia(
                                        ...
                                        includeInReport = entity.includeInReport
                                    )
                                    ...
                                    if (response != null) {
                                        ...
                                        callback.onError(
                                            id = id,
                                            ex = TruvideoSdkException(message = "Request not found")
                                        )
                                    }
